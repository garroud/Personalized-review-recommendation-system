...
import parse_review_TripAdvisor as prt
...
def random_test():
    users = [f[:-4] for f in os.listdir('./users_TA')]
    prods = [f[:-4] for f in os.listdir('./products_TA')]
    random.seed()
    while (True):
        user_id = users[random.randrange(len(users)-1)]
        prod_id = prods[random.randrange(len(prods)-1)]
        is_valid = True
        with open("products_TA/"+prod_id+".txt",'r') as f:
            for line in f:
...
if check == 'y':
    ...
    #shutil.rmtree('users_TA')
    #shutil.rmtree('products_TA')
    #os.remove('products_distribution_TA.txt')
    #os.remove('parsed_reviews_TA.txt')
    #os.mkdir('users_TA')
    #os.mkdir('products_TA')
    #prt.build_file_sys()
...
if is_t == 'y':
    ...
    #ud.get_user_distribution(check_t, date.today(), rate,'_TA')
    #pd.get_prod_distribution(is_t,date.today(),rate,'_TA')
else:
    pass
    #ud.get_user_distribution(check_t,date.today(),0,'_TA')
    #pd.get_prod_distribution(check_t, date.today(), 0,'_TA')
while True:
    ...
    result_1 = iPTRS.iPTRS_derivation(user_id, prod_id, 'P', 3, 0.001,check_t,date.today(),rate,'_TA','u')
    result_2 = iPTRS.iPTRS_derivation(user_id, prod_id, 'P', 3, 0.001,check_t,date.today(),rate,'_TA','i')
    print ('user_id: %s , prod_id: %s' % (user_id,prod_id))
    print (result_1)
    print (result_2)
