...
def get_prod_distribution(is_t, present, rate, type):
    topics = []
    if type == "_Yelp":
        topics = basic_info.topics_Y
    else:
        topics = basic_info.topics_TA
    file_list = os.listdir('products'+type)
    with open("parsed information/products_distribution"+type+".txt","w+") as f:
        for the_file in file_list:
            f.write(the_file[:-4])
            size = 0
            distribution = np.zeros(len(topics))
            with open("products" + type + '/'+ the_file, 'r') as finput:
                for line in finput:
                    time   = line.split(' ')[2]
